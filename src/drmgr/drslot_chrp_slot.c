...
remove_slot(struct dr_node *node)
{
	...
	if (rc) {
		...
		if (rc2 && rc2 != -EINVAL) {
			err_msg("Unknown failure %d. Data may be out of sync "
				"and\nthe system may require a reboot.\n", rc2);
			return rc;
		}
		...
		if (num_acquired) {
			...
			if (rc2) {
				err_msg("failed to re-enable hotplug "
					"children. %d\n", rc2);
				return rc;
			}
		}
...
add_slot(struct options *opts)
{
	...
	if (n_children) {
		...
		if (rc) {
			err_msg("Configure adapter failed.\n");
			if (release_hp_children(node->drc_name)) {
				err_msg("Unknown failure. Data may be out of "
					"sync and\nthe system may require "
					"a reboot.\n");
		...
		dbg("adapter in node[%s] has been configured.\n",
		    node->drc_name);
	}
...
drslot_chrp_slot(struct options *opts)
{
	...
	if (! slot_dlpar_capable()) {
		err_msg("DLPAR slot operations are not supported on"
			"this kernel.");
		return -1;
	}
	...
	switch (opts->action) {
		...
		if (node && node->is_owned) {
			err_msg("partition already owns %s\n",
				opts->usr_drc_name);
			rc = RC_ALREADY_OWN;
		} else {
			rc = add_slot(opts);
